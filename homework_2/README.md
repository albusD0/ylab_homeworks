...
Баг при значении **-1** довольно маловероятен, и все же, никогда неизвестно наверняка, как именно программист реализовал алгоритм рассчета скидки, ошибка может закрасться не в сами числа, которые он использовал, а в знаки (>, <, =) или формулу (способ) подсчета, поэтому необходимо проверить и это значение тоже. Также это значение позволит реализоватеь негативный тест (как и значение **2002**) - верно ли отрабатывает система или просто падает, выдает ли предупреждение о недопустимости ввода таких значений.
...
**Если бы требования к рассчету скидки были бы четкими, нерасплывчатыми, то и набор тестовых данных был бы однозначным и уместился бы в пару табличек**
