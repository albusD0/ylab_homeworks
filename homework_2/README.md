...
В задании есть два спорных момента:

> От `0` до 100 баллов - скидка 1%

> От `500` до 2000 баллов - скидка 3 %

В случае с нулем нужно, на мой взгляд, его исключить, так как 0 - это отсутствие баллов, а значит скидка не должна предоставляться. 

Что касается:

> От `500` до 2000 баллов - скидка 3 %

это либо ошибка, либо данные верны и в этом случае предыдущее `От 101 до 500 баллов` нужно воспринимать как `не включительно`.

Я все же приведу несколько вариантов.

**`Вариант 1`** - ноль принимаем, данные в `до 500 баллов` верны - ***до*** означает `не включительно`, поэтому верхней границей для скидки в `3 %` станет `499`

| Значения для проверки	| Скидка |
| - | - |
| `-1, 0, 1`; `99, 100, 101` | 1% | 
| `100, 101, 102`; `498, 499, 500` | 3% |
| `499, 500, 501`; `1999, 2000, 2001` | 5% |
| `2000, 2001, 2002` | 10% |

Баг при значении **-1** довольно маловероятен, и все же, никогда неизвестно наверняка, как именно программист реализовал алгоритм рассчета скидки, ошибка может закрасться не в сами числа, которые он использовал, а в знаки (>, <, =) или формулу (способ) подсчета, поэтому необходимо проверить и это значение тоже. Также это значение позволит реализовать негативный тест (как и значение **2002**) - верно ли отрабатывает система или просто падает, выдает ли предупреждение о недопустимости ввода таких значений.

При тестировании можно сократить количество проверок, убрав повторяющиеся значения, при этом внимательно отслеживая, какой именно класс мы тестируем и что получаем на выходе.
...
**`Вариант 2`** - ноль не принимаем, данные в `до 500 баллов` верны - ***до*** означает `не включительно` (верхняя граница для `3 %` - `499`)
...
Другой набор тестовых данных, если брать значение самой границы и одно значение за ней:
...
**`Вариант 3`** - если в *`От 500 до 2000 баллов - скидка 3 %`* закралась ошибка и в действительности нужно отсчитывать от `501`.

Набор тестовых значений с учетом 0 баллов
...
Набор тестовых значений без учета 0 баллов
...
Наборы альтернативных тестовых данных, если брать значение самой границы и одно значение за ней:
...
#### PS: Если бы требования к рассчету скидки были бы четкими, нерасплывчатыми, то и набор тестовых данных был бы однозначным и уместился бы в пару табличек. Мое личное предпочтение: 0 баллов не учитывается при рассчете скидки, а отсчет от 500 принял бы за ошибку (Вариант 3 без учета нуля баллов) - эти моменты я предложил бы учесть команде разработки при создании приложения, реализующего систему скидок в зависимости от накопленных баллов
...
- ***Form*** - форма для отправки данных, вводимых пользователем, на сервер (в базу данных)
...
- ***Dropdown*** - выпадающий список уже имеющихся вариантов на выбор
