...
| | *Предусловие: имеются письма в одном из разделов "Входящие" или "Отправленные". При отсутствии отправить на текущий тестовый адрес письмо либо создать по следующей схеме:  1. Нажать кнопку "Создать письмо".* *2. Заполнить поля "Кому", "Тема" и тело сообщения.* *3. Нажать кнопку "Отправить письмо"*| | |
| |  | 2. Нажать кнопку "Удалить письмо" | Письмо успешно удаляется, при этом выдается сообщение, действительно ли пользователь собирается удалить его |
